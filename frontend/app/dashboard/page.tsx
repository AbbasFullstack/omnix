...
export default function Home() {
  ...
  useEffect(() => {
    const handleUser = async (u: any) => {
      ...
      if (u) {
        ...
        // Save GitHub provider_token from session into user_github table
        const sess = await supabase.auth.getSession();
        const providerToken = sess.data.session?.provider_token;
        if (providerToken && githubRow?.user_id === u.id) {
          await supabase.from('user_github').update({ access_token: providerToken }).eq('user_id', u.id);
        }
      }
    };
  ...
  const openRepos = async () => {
    ...
    try {
      await loadImported();
      const sess = await supabase.auth.getSession();
      let token = sess.data.session?.provider_token || '';
      // Fallback: use stored access_token from user_github table
      if (!token && githubRow?.access_token) token = githubRow.access_token;
      const headers: any = { Accept: 'application/vnd.github.v3+json' };
      if (token) headers.Authorization = 'Bearer ' + token;
      let res = token ? await fetch('https://api.github.com/user/repos?per_page=100&sort=updated', { headers }) : null;
      if (!res || !res.ok) {
        // 401 means token expired - show reconnect message
        if (res?.status === 401) {
          alert('⚠️ GitHub dobara connect karein (token expire ho gaya)');
          setReposOpen(false);
          return;
        }
        res = await fetch('https://api.github.com/users/' + githubRow.login + '/repos?per_page=100&sort=updated', { headers });
      }
  ...
  const fetchRepoDetails = async (repo: any) => {
    ...
    // Fetch tree for file count
    const branch = repo.default_branch || 'main';
    let fileCount = 0;
    let sizeMB = repo.size ? Math.round(repo.size / 1024) : 0;
    ...
    try {
      ...
      if (treeRes.ok) {
        ...
        fileCount = treeData.tree?.filter((f: any) => f.type === 'blob').length || 0;
      }
    } catch {}
    ...
    setRepoDetails({
      ...
      size_mb: sizeMB,
      languages,
      updated_at: repo.updated_at ? new Date(repo.updated_at).toLocaleDateString() : '—',
      created_at: repo.created_at ? new Date(repo.created_at).toLocaleDateString() : '—',
    });
  };
  ...
  return (
    <div className="h-screen bg-[#0a0a0a] text-white flex flex-col overflow-hidden">
              ...
              <div className={`max-w-[85%] px-4 py-3 rounded-2xl text-sm leading-relaxed whitespace-pre-wrap ${m.role === 'user' ? 'bg-orange-600/20 border border-orange-500/30 rounded-br-md' : 'bg-white/[0.04] border border-white/[0.07] rounded-tl-md'}`}>
                ...
                {m.text.split('```').map((part, idx) => {
                  if (idx % 2 === 1) {
                    ...
                    return (
                      <div key={idx} className="my-2 rounded-xl bg-[#0a0a0a] border border-white/10 overflow-hidden">
                        <div className="flex items-center justify-between px-3 py-1.5 bg-white/5 border-b border-white/10">
                          <span className="text-[9px] font-bold text-white/50 uppercase">{lang || 'code'}</span>
                          <button
                            onClick={() => navigator.clipboard.writeText(code)}
                            className="text-[9px] font-bold text-orange-300 hover:text-orange-200"
                          >
                            Copy
                          </button>
                        </div>
                        <pre className="p-3 overflow-x-auto text-[10px] text-white/80 bg-[#0a0a0a]"><code>{code}</code></pre>
                        <div className="px-3 py-2 border-t border-white/10 bg-white/5">
                          <button
                            onClick={async () => {
                              const sess = await supabase.auth.getSession();
                              const token = sess.data.session?.provider_token || githubRow?.access_token || '';
                              if (!token || !activeRepo) {
                                alert('⚠️ Pehle GitHub repo import karein aur active karein');
                                return;
                              }
                              const path = prompt('File path (e.g., src/app/page.tsx):', '');
                              if (!path) return;
                              const commitMsg = prompt('Commit message:', 'AI update via OmniX');
                              if (!commitMsg) return;
                              try {
                                const b64Content = btoa(code);
                                const res = await fetch(`https://api.github.com/repos/${activeRepo.full_name}/contents/${path}`, {
                                  method: 'PUT',
                                  headers: {
                                ...
                                if (res.ok) alert('✅ Commit successful! Check GitHub.');
                                else alert('⚠️ Error: ' + (data.message || 'Commit fail'));
                              } catch (e: any) {
                                alert('⚠️ Push error: ' + e.message);
                              }
                            }}
                            className="text-[9px] font-bold text-emerald-300 hover:text-emerald-200"
                          >
                            Push to GitHub →
                          </button>
                        </div>
                ...
                <div key={r.id} className="p-3 rounded-xl bg-white/[0.04] border border-white/[0.07]">
                  <div className="flex items-start gap-2">
                    ...
                    <div className="flex flex-col gap-1 shrink-0">
                      <button onClick={() => fetchRepoDetails(r)} className="text-[8px] font-bold text-blue-300 border border-blue-500/40 rounded-lg px-2 py-1">Details</button>
      ...
      {repoDetails && (
        <div className="fixed inset-0 z-[60] bg-black/90 backdrop-blur-sm flex items-end" onClick={() => setRepoDetails(null)}>
          <div className="w-full bg-[#151515] border-t border-white/10 rounded-t-3xl p-4 pb-8 max-h-[70vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="w-10 h-1 mx-auto mb-4 rounded-full bg-white/20" />
            <div className="flex items-center gap-2 mb-3">
              <GitBranch className="w-4 h-4 text-emerald-400" />
              <p className="text-sm font-bold flex-1">{repoDetails.full_name}</p>
              <button onClick={() => setRepoDetails(null)} className="p-2 rounded-lg bg-white/5"><X className="w-4 h-4" /></button>
            </div>
            <div className="space-y-3">
              {repoDetails.description && <p className="text-[10px] text-white/60">{repoDetails.description}</p>}
              <div className="grid grid-cols-2 gap-2">
                <div className="p-3 rounded-xl bg-white/[0.04] border border-white/[0.07]">
                  <p className="text-[8px] text-white/30 uppercase">Files</p>
                  <p className="text-[11px] font-bold text-white/80">{repoDetails.file_count}</p>
                </div>
                <div className="p-3 rounded-xl bg-white/[0.04] border border-white/[0.07]">
                  <p className="text-[8px] text-white/30 uppercase">Size</p>
                  <p className="text-[11px] font-bold text-white/80">{repoDetails.size_mb} MB</p>
                </div>
                <div className="p-3 rounded-xl bg-white/[0.04] border border-white/[0.07]">
                  <p className="text-[8px] text-white/30 uppercase">Stars</p>
                  <p className="text-[11px] font-bold text-white/80">{repoDetails.stargazers_count}</p>
                </div>
                <div className="p-3 rounded-xl bg-white/[0.04] border border-white/[0.07]">
                  <p className="text-[8px] text-white/30 uppercase">Forks</p>
                  <p className="text-[11px] font-bold text-white/80">{repoDetails.forks_count}</p>
                </div>
              </div>
              <div className="p-3 rounded-xl bg-white/[0.04] border border-white/[0.07]">
                <p className="text-[8px] text-white/30 uppercase mb-1">Languages</p>
                <div className="flex flex-wrap gap-1">
                  {repoDetails.languages.length > 0 ? repoDetails.languages.map((lang: string) => (
                    <span key={lang} className="text-[9px] font-bold text-emerald-300 bg-emerald-500/10 px-2 py-1 rounded-lg">{lang}</span>
                  )) : <span className="text-[9px] text-white/40">—</span>}
                </div>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div className="p-3 rounded-xl bg-white/[0.04] border border-white/[0.07]">
                  <p className="text-[8px] text-white/30 uppercase">Updated</p>
                  <p className="text-[10px] font-bold text-white/80">{repoDetails.updated_at}</p>
                </div>
                <div className="p-3 rounded-xl bg-white/[0.04] border border-white/[0.07]">
                  <p className="text-[8px] text-white/30 uppercase">Created</p>
                  <p className="text-[10px] font-bold text-white/80">{repoDetails.created_at}</p>
                </div>
              </div>
              <a href={repoDetails.html_url} target="_blank" rel="noopener noreferrer" className="block w-full py-2.5 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 text-[10px] font-bold text-center">
                GitHub par dekhein →
              </a>
            </div>
          </div>
